...
class Scanner(in: Reader[Char]) extends Reader[Tokens.Token] with Reporter {
  ...
  // List of keywords
  val isKeyword    = Set("if", "else", "val", "var", "while", "=>", "def", "new", "Array")
  ...
  def getName() = {
    ...
    // Check if the string is exactly "true". If so, return a Literal(true) otherwise,
    // it returns a Literal(false)
...
class BaseParser(in: Scanner) extends Parser(in) {
   ...
   *
   * We expect a token Ident(x) where x is the type and we'll match it to one of
   * the predefined types (Int, Unit, Boolean).
   */
   ...
   *
   * The grammar for an optional type is: <ident> [: <type>] '=',
   * so if we see a colon -> Delim(':') we consume it and parse the type.
   */
...
class SyntacticSugarParser(in: Scanner) extends BaseParser(in) {
  ...
  override def parseSimpleExpression = in.peek match {
    // If the next token is an if, we parse the if statement as we did before
    case Keyword("if") =>
      val pos = accept("if").pos
      accept('(')
      val cond = parseSimpleExpression
      ...
      // Instead of just accepting else we just parse the else branch if it exists
    ...
    // Not an if statement, we parse the simple expression just as before
  ...
  override def parseExpression = {
    ...
    // If the next token is a Delim(';'), that means that there is another expression to parse.
    if (in.hasNext(isNewLine)) {
      accept(';') // consume the semicolon
      Let(freshName("tmp"), UnknownType, res, parseExpression).withPos(pos)
    } else {
      // If there is no semicolon, we return the parsed expression.
      res
    }
  }
...
class FunctionParser(in: Scanner) extends SyntacticSugarParser(in) {
  ...
  override def parseType = in.peek match {
    case Delim('(') =>
      // <type> ::= '('[<type>[','<type>]*]')' '=>' <type>
      accept('(')
      // Use parseList to create a list of types separated by ',' and terminated by ')'
      val types = parseList(parseType, ',', tok => tok != Delim(')'), true)
      accept(')')
      accept("=>")
      // Parse the return type
      val rtp = parseType
      // Build the function type with the list of arguments and the return type
      FunType(types.map(tp => ("", tp)), rtp)

    case _ => 
      // Otherwise, we parse a simple type as before
      val base = super.parseType

      // If the next token is '=>', we parse the function type
      if (in.hasNext(_ == Keyword("=>"))) {
        // <type> ::= <type> '=>' <type>
        accept("=>")
        val rtp = parseType
        FunType(List(("", base)), rtp)
      } else {
        // <type> ::= <ident>
        base
      }
  ...
  def parseFunction: Exp = {
    ...
    val defPos = accept("def").pos
    val (name, pos) = getName
    accept('(')

    // Parse the list of arguments separated by ',' (if there are any)
    // val args = parseList[Arg](parseArg, ',', tok => tok != Delim(')'))
    val args = parseList[Arg](parseArg, ',', tok => tok match {
      case Delim(')') => false // means that when we reach the end of the list, we stop
      case _ => true
    })
    accept(')')
    ...
    // accept(';') // we don't need to accept the semicolon as it is done in parseProgram
    // Build the function definition choosing defPos as the position of the function (def keyword)
    FunDef(name, args, rtp, body).withPos(defPos)
  ...
  def parseProgram = in.peek match {
    // <prog> ::= ['def' <ident> '(' [<arg>[','<arg>]*] ')' [':' <type>] '=' <simp> ';']* <exp>
    // If the next token is a function definition, we parse it
    case Keyword("def") =>
      // Parse the list of functions while the next token is a function definition
      val funcs = parseList[Exp](parseFunction, ';', tok => tok == Keyword("def"))

      accept(';') // accept the semicolon after the last function
      // Build the LetRec node with the list of functions and the main expression
      LetRec(funcs, parseExpression)

    // If the next token is not a function definition, we parse the expression
  ...
  def parseTight = in.peek match {
    ...
    case  _ =>
      var res = parseAtom
      while (in.hasNext(_ == Delim('('))) {
        val posApp = in.next().pos // consume the '('
        val args = parseList[Exp](parseSimpleExpression, ',', tok => tok != Delim(')'))
        accept(')')
        res = App(res, args).withPos(posApp)
      }
      res
  }
 ...
 * arr(0) = 3;
 *
 * The access is going to be parsed as a function application but this is fine.
...
class ArrayParser(in: Scanner) extends FunctionParser(in) {
  ...
  override def parseTight = in.peek match {
    ...
    case  _ =>
      ...
      while (in.hasNext(_ == Delim('('))) {
        ...
        val args = parseList[Exp](parseSimpleExpression, ',', tok => tok != Delim(')'))
        accept(')')
        if (args.size == 1 && in.hasNext(_ == Delim('='))) {
          val posEq = in.next().pos // consume the '=' and track position
          val rhs = parseSimpleExpression
          // Transform arr(idx) = rhs to Prim("block-set", List(arr, idx, rhs))
          // args.head gets the first element of the list (args) and rhs is the value to update
          res = Prim("block-set", List(res, args.head, rhs)).withPos(posEq)
        } else {
          // Otherwise, we are in a function application
          res = App(res, args).withPos(posApp)
        }
      }
      res
   ...
   */
  override def parseSimpleExpression = in.peek match {
    // <simp> ::= 'new' 'Array' '['<type> ']' '('<simpl>')'
    case Keyword("new") =>
      val pos = accept("new").pos // consume 'new' and track position
      accept("Array")
      accept('[')
      val tp = parseType
      accept(']')
