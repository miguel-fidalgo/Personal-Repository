...
class StackInterpreter extends Interpreter with BugReporter {
  ...
  def eval(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case FunDef(_, args, _, fbody) =>
      ...
      // - the list of parameters names (args)
      // - the body of the function (fbody)
      // - the current environment
      memory(sp) = Func(args map { arg => arg.name }, fbody, env)
    case LetRec(funs, body) =>
      // Allocate memory for all functions
      val n = funs.length
      val funcsList: List[(String, Int)] = funs.zipWithIndex map {
        case (fun@FunDef(name, _, _, _), i) =>
          val loc = sp + i
          // Evaluate each function definition into its allocated memory
          eval (fun, loc)(env)
          (name, loc)
      }
      // Update each function closure with the mutually recursive functions
      funcsList foreach {
        case (_, loc) =>
          memory(loc) match {
            case f: Func => f.withVals(funcsList)
            case _ => BUG(s"LetRec: not a function")
          }
      }
      // Extend the environment with the functions
      val env2 = env.withVals(funcsList)
      // Evaluate the body with the new environment 
      eval(body, sp + n)(env2)
      // Copy the body’s value from sp + n back to sp
      memory(sp) = memory(sp + n)
    case App(fun, args) =>
      // Evaluate the function expression into memory(sp)
      eval(fun, sp)(env)
      // Evaluate each argument into consecutive locations starting at sp+1.
      val base = sp + 1
      for ((arg, i) <- args.zipWithIndex)
        eval(arg, base + i)(env)
      // Now, inspect the function value stored at memory(sp)
      memory(sp) match {
        case Func(fargs, fbody, fenv) =>
          if (fargs.length != args.length)
            BUG("Wrong number of arguments in function application")
          // Build a list of (parameter name, location) pairs.
          val newBindings: List[(String, Int)] =
            fargs.zipWithIndex.map { case (name, i) => (name, base + i) }
          // Choose a new location for evaluating the function body.
          val newSp = sp + args.length + 1
          // Evaluate the function body using the closure's environment extended
          // with the new bindings.
          eval(fbody, newSp)(fenv.withVals(newBindings))
          // Copy the result from newSp to sp so that the App expression yields the result.
          memory(sp) = memory(newSp)
        case Primitive("getchar") =>
          memory(sp) = Cst(Console.in.read)
        case Primitive("putchar") =>
          val argLoc = base
          memory(sp) = {
            val Cst(c: Int) = memory(argLoc)
            Console.out.write(c)
            Console.out.flush
            Cst(())
          }
        case _ => BUG("Application of non-function")
    }
    case ArrayDec(size, _) =>
      // First evaluate the size of the array and store it at sp
      eval(size, sp)(env)
      memory(sp) match {
        case Cst(s: Int) => memory(sp) = Cst(new Array[Any](s))
        case _ => BUG(s"ArrayDec: not an integer")
      }
  }
}
