...
abstract class X86Compiler extends BugReporter with Codegen {
  ...
  // Global variable to store the last condition operator
  private var lastCondOp: Option[String] = None
  ...
  def trans(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case Let(x, a, b) =>
      trans(a, sp)(env)
      trans(b, sp + 1)(env.withVal(x, sp)) // x -> sp
      emitln(s"movq ${regs(sp + 1)}, ${regs(sp)}")

    case Ref(x) =>
      emitln(s"movq ${regs(env(x))}, ${regs(sp)}")

    case Cond(op, l, r) =>
      trans(l, sp)(env)
      trans(r, sp + 1)(env)
      lastCondOp = Some(op) // Store the op so that we can use it in If

    case If(cond, tBranch, eBranch) =>
      trans(cond, sp)(env)
      emitln(s"cmpq ${regs(sp + 1)}, ${regs(sp)}")
      
      // Create labels for if_then and if_end
      val lab = freshLabel("if")

      // If condition is satisfied, then jump to if_then
      val op = lastCondOp.getOrElse {
        BUG("No condition operator found after Cond(...)!")
      }
      transJumpIf(op)(lab + "_then")
      ...
      trans(tBranch, sp)(env) // then branch
    ...
    case VarDec(x, rhs, body) =>
      ...
      trans(rhs, sp)(env)
      trans(body, sp + 1)(env.withVal(x, sp)) // x -> sp
      emitln(s"movq ${regs(sp + 1)}, ${regs(sp)}")

    case VarAssign(x, rhs) =>
      // Mutable variable assignment
      trans(rhs, sp)(env)
      emitln(s"movq ${regs(sp)}, ${regs(env(x))}")
    ...
    case While(cond, lBody, body) =>
      ...
      // If the condition is satisfied, then jump to the body, if not jump to the end
      val op = lastCondOp.getOrElse {
        BUG("No condition operator found after Cond(...)!")
      }
      emitln(s"cmpq ${regs(sp + 1)}, ${regs(sp)}")
      transJumpIf(op)(lab + "_body")
