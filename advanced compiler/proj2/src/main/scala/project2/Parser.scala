...
object Tokens {
  ...
  case object EOF extends Token // end of file
  case class Number(x: Int) extends Token // number: [0-9]+
  case class Ident(x: String) extends Token // identifier: [a-zA-Z][a-zA-Z0-9]*
  case class Keyword(x: String) extends Token // keyword: val
  case class Delim(x: Char) extends Token // delimiter: '=', ';'
  // Ignore whitespace: ' ', '\n', '\r', '\t'
}
...
class Scanner(in: Reader[Char]) extends Reader[Tokens.Token] with Reporter {
   ...
   */
  def getNum() = {
    var res = 0 // var is a mutable variable
...
class ArithParser(in: Scanner) extends Parser(in) {
  ...
  def parseExpression(min: Int): Exp = {
    ...
    var left = parseUAtom // parse the unary or parenthesized expression
    // We check if the next token is an operator with precedence >= min
    while (in.hasNext(isInfixOp(min))) {
      val (op, opPos) = getOperator
      val curPrec = prec(op) // get the current level of precedence

      // Parse recursively the right-hand side, but with a higher precedence
      // from left to right (left associative)
      val right = parseExpression(curPrec + assoc(op))

      // Combine it into the AST node
      left = Prim(op, left, right).withPos(opPos)
    }
 ...
 * <simp>  ::= <uatom>[<op><uatom>]*
 * <exp>   ::= <simp>
 *           | 'val' <ident> '=' <simp>';' <exp> // introduce immutble variable (val)
 */
class LetParser(in: Scanner) extends ArithParser(in) {
  ...
  // The simple expression is the past Expression in ArithParser
...
class VariableParser(in: Scanner) extends BranchParser(in) {
  ...
  override def parseExpression = in.peek match {
    ...
    // <simp> or 'val' <ident> '=' <simp>';' <exp>
    case _ => super.parseExpression
  }

  override def parseSimpleExpression = (in.peek, in.peek1) match {
    // <ident> '=' <simp>
    /* This is parsed with in.peek and in.peek1 what means look at the next two
    tokens. If the first one is Ident(...) and the second one Delim('='), then
    we parse it as an assignment statement of a mutable variable */
    case (Ident(_), Delim('=')) =>
      val (name, pos) = getName
      accept('=')
      val rhs = parseSimpleExpression
    ...
    // <uatom>[<op><uatom>]* or 'if' '('<cond>')' <simp> 'else' <simp>
    case _ => super.parseSimpleExpression
  }
}
...
class LoopParser(in: Scanner) extends VariableParser(in) {
  ...
  /* We just need to modify the pareExpression since the rest of the grammar
  remains the same. Parsing it is similar to the if statement */
  override def parseExpression = in.peek match {
    // 'if' '('<cond>')' <simp> 'else' <simp>
    // 'while' '('<cond>')'<simp>';' <exp>
    case Keyword("while") =>
      ...
      val cond = parseCondition
      accept(')')
      val lbody = parseSimpleExpression // loop body is the content inside the loop
      accept(';')
      val body = parseExpression // body is the content after the loop
      While(cond, lbody, body)
