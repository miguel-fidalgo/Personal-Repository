...
class CPSOptimizer_Greybox extends CPSOptTest {
  ...
  @Test def testFunInliningCrasher =
    testCPSOptEarly("def f(x: Int): Int = if (x == 0) 0 else g(x - 1); def g(y: Int): Int = f(y); def h(z: Int) = z; f(0)", _.get(AppF) == 0)

  @Test def testFunInliningRecursiveTrick =
    testCPSOptEarly("def f(x: Int): Int = g(x); def g(y: Int): Int = f(y); def h(z: Int) = z; h(3)", _.get(AppF) == 0)
