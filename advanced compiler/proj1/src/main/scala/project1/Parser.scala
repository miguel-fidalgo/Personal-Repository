...
class SingleAddOpParser(in: Reader[Char]) extends SingleDigitParser(in) {
  ...
  override def parseExpression: Exp = {
    val lop = parseTerm
    // in.peek looks the next character without removing (advancing) it
    in.peek match {
      case '+' => parseAdd(lop)
      case '-' => parseSub(lop)
 ...
 * precedence rules. However, we are going to see that we can
 * do it quite easily.
 *
...
class ArithParOpParser(in: Reader[Char]) extends ArithOpParser(in) {
  ...
  override def parseFactor: Exp = {
    in.peek match {
      ...
      case '(' =>
        // First we accept (, then we parse the expression inside, accept last
        // ) and return expression parsed 
        accept('(')
