...
abstract class StackASMGenerator extends Codegen {
  ...
  def emitCode(exp: Exp): Unit = exp match {
    ...
    case Div(x,y) =>
      emitCode(y)
      emitln("pushq %rax")
      emitCode(x) // Dividend in %rax
      emitln("cqto") // To extend dividend to rdx:rax
      emitln("popq %rbx") // Divisor in %rbx
      emitln("idivq %rbx") // Perform division: %rdx:rax / %rbx = %rax mod %rdx
  }
}
